...
USER_TEMPLATE = (
    ...
    "- polygon_get_news(ticker, limit): Returns 'results' array with news items\n"
    "  Example: extract: ['articles = results[]'], accept_if: ['len(articles) > 0']\n"
    "  WRONG: extract: ['articles[]'] - 'articles' field doesn't exist in Polygon response\n"
    "  RIGHT: extract: ['articles = results[]'] or ['results[]'] - use alias or correct path\n"
    ...
    "  Example: extract: ['price', 'changesPercentage', 'volume'], accept_if: ['price is not None', 'changesPercentage is not None', 'volume > 0']\n"
    "  WRONG: accept_if: ['len(result) > 0'] - 'result' is never extracted\n"
    "- fmp_get_company_profile(symbol): Extract 'description', 'industry', 'sector'\n"
    "  Example: extract: ['description', 'industry'], accept_if: ['description is not None', 'industry is not None']\n"
